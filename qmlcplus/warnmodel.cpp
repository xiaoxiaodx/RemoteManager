...
WarnModel::WarnModel(QObject *parent ):QAbstractListModel(parent)
{
    ...
    m_listWarn.append(new WarnModelData(true,"19911103","1111111","sasads","aaaaa","444","333"));
    m_listWarn.append(new WarnModelData(true,"19911103","1111111","sasads","aaaaa","444","333"));
    m_listWarn.append(new WarnModelData(true,"19911103","1111111","sasads","aaaaa","444","333"));
    m_listWarn.append(new WarnModelData(true,"19911103","1111111","sasads","aaaaa","444","333"));
}
...
QVariant WarnModel::data(const QModelIndex &index, int role) const
{
    ...
    else if(role == PersonSerial)
        return QVariant::fromValue(m_listWarn.at(index.row())->personSerial());
    else if(role == PersonName)
        return QVariant::fromValue(m_listWarn.at(index.row())->personName());
    else if(role == DeviceChannel)
        return QVariant::fromValue(m_listWarn.at(index.row())->deviceChannel());
    else if(role == DeviceName)
        return QVariant::fromValue(m_listWarn.at(index.row())->deviceName());
    return QVariant();
}
...
QHash<int, QByteArray> WarnModel::roleNames() const
{
    ...
    roles[PersonSerial]="personSerial";
    roles[PersonName]="personName";
...
bool WarnModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
    ...
    if (data(index, role) != value) {
        if(role == IsSelect)
            ...
            modelData->setabsolutePath(value.toString());
        else if(role == PersonName)
            modelData->setpersonName(value.toString());
        else if(role == PersonSerial)
            modelData->setpersonSerial(value.toString());
        else if(role == DeviceChannel)
...
void WarnModel::funDeleteIndex(int index)
{
    ...
    if(file.open(QIODevice::WriteOnly)){
        for (int i=m_listWarn.size()-1;i>=0;i--) {
            ...
            QString imgInfoStr = modelData->absolutePath()+" "+modelData->warnTemp()+" "+modelData->imgName()+".jpg";
...
void WarnModel::funFlushWarnInfo(QString capturePath,QString logFileName)//这里文件名就是日期"yyMMdd"
{
    ...
    if(file.open(QIODevice::ReadOnly)){
        ...
        while (!warnStr.isNull()) {
            ...
            QString datetime = fileName.remove(".jpg");
...
#include <QFile>
void WarnModel::funAppendWarnInfo(QString path,QString channel,QString devicename,QVariantMap map,QByteArray arrimg)
{


    qDebug()<<"funAppendWarnInfo    "<<channel<<"   "<<devicename<<"    "<<map;

    QDateTime curDateTime =  map.value("datetime").toDateTime();
    QString  curDatetimeStr = curDateTime.toString("yyyyMMdd_hhmmss");
    QString desFileDir = path+"/image";
    QString imgAbsolutePath = path+"/image/"+curDatetimeStr+".jpg";
    ...
    QFile fileimg(imgAbsolutePath);
    if(fileimg.open(QIODevice::WriteOnly)){
        fileimg.write(arrimg.data(),arrimg.length());
        fileimg.close();

        QString datestr = QDate::currentDate().toString("yyyyMMdd");
        float warnTemp = map.value("alarmTemperature").toFloat();
        //存报警图片信息
        ...
        */
        ...
        if(imgInfofile.open(QIODevice::WriteOnly | QIODevice::Text | QIODevice::Append)){
            ...
            QString imgInfoStr = imgAbsolutePath+" "+tempStr+" "+curDatetimeStr+".jpg";
            ...
            if(curDate.compare(datestr)==0){
                ...
                m_listWarn.insert(0,new WarnModelData(curSelect,date+" "+time,tempStr,curDatetimeStr,imgAbsolutePath,"","",channel,devicename));
                endInsertRows();
            }
...
bool WarnModel::funScreenShoot(QString path,QQuickWindow *quic,int capx,int capy,int capw,int caph,float warnTemp)
{
    ...
    if(!img1.save(curDatetimeStr+".jpg","jpg")){
